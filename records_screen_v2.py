...
class RecordDetailDialog(QDialog):
    def __init__(self, record_id=None, parent=None, editable=False):
    ...
    def save_record(self):
        ...
        try:
            if self.record_id:
                ...
                cursor.execute("""
                    INSERT INTO Records (Patient_Name, Doctor_Name, Date, Time, Description, Prescription)
                    VALUES (?, ?, ?, ?, ?, ?)
                """, (self.patient_name.text(), self.doctor_name.text(), self.date.date().toString("yyyy-MM-dd"), 
                      self.time.time().toString("HH:mm:ss"), self.description.toPlainText(), 
...
class RecordsScreen(BaseScreen):
    ...
    def load_data(self):
        ...
        try:
            conn = sqlite3.connect('CallADoctor.db')
            cursor = conn.cursor()
            cursor.execute("SELECT Record_ID, Patient_Name, Doctor_Name, Date FROM Records")
