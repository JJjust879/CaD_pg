...
class AppointmentDetailDialog(QDialog):
    def __init__(self, appointment_id=None, parent=None, editable=False):
...
class AppointmentsScreen(BaseScreen):
    ...
    def load_data(self):
        ...
        try:
            conn = sqlite3.connect('CallADoctor.db')
            cursor = conn.cursor()
            cursor.execute("SELECT Appointment_ID, Patient_Name, Doctor_Name, Appointment_Date, Appointment_Approval FROM Appointments")
