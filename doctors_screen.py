    ...
    def save_doctor(self):
        ...
        try:
            ...
            if self.doctor_id:
                ...
                cursor.execute("""
                    INSERT INTO Doctor (Clinic_ID, Doctor_Name, Doctor_Specialization, Doctor_Age, 
                                        Doctor_Gender, Doctor_Email, Doctor_Phone, Doctor_Portrait)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                """, (self.clinic_id, self.name.text(), self.specialization.text(), self.age.value(), 
...
class DoctorsScreen(BaseScreen):
    ...
    def load_data(self):
        ...
        try:
            conn = sqlite3.connect('CallADoctor.db')
            cursor = conn.cursor()
            cursor.execute("SELECT Doctor_ID, Doctor_Name, Doctor_Specialization, Doctor_Age FROM Doctor WHERE Clinic_ID = ?", (self.clinic_id,))
